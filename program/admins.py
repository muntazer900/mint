...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("❌ ماكو شي مشتغل يحمار")
        elif op == 1:
            await m.reply("✅ __قائمة الانتضار__ **قارفة.**\n\n**• تم خروجي من المحادثة الصوتية**")
        elif op == 2:
            await m.reply("🗑️ **تم مسح قوائم الانتضار**\n\n**• وتم مغادرة الدردشة الصوتية**")
        else:
            await m.reply_photo(
                photo=f"{IMG_3}",
                caption=f"⏭ **اهلين عيني المشرف ابشر تم التخطي.**\n\n🏷 **الاسم:** [{op[0]}]({op[1]})\n💭 **الدردشة:** `{chat_id}`\n💡 **الحاله:** `مشغل طبيعي`\n🎧 **طلب من:** {m.from_user.mention()}",
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("✅ **اهلين عيني المشرف ابشر تم الايقاف.**")
        except Exception as e:
            await m.reply(f"🚫 **error:**\n\n`{e}`")
    else:
        await m.reply("❌ **ماكو شي مشتغل دعبل منا**")
...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "⏸ **تم ايقاف الاغنية مؤقتا.**\n\n• **لاستمرار الاغنية, ارسل الامر**\n» .استمرار "
            )
        except Exception as e:
            await m.reply(f"🚫 **error:**\n\n`{e}`")
    else:
...
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "▶️ **تم استمرار المسار.**\n\n• **لايقاف المسار مره اخرى,ارسل الامر**\n» .توقف "
            )
        except Exception as e:
            await m.reply(f"🚫 **error:**\n\n`{e}`")
    else:
...
@Client.on_message(
    command(["افتح", f"unmute@{BOT_USERNAME}", "vunmute"]) & other_filters
)
@authorized_users_only
...
async def cbpause(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        ...
        await query.answer("❌ماكو شي مشتغل يحمار", show_alert=True)
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("✅ **تم حب انتهى البث**", reply_markup=bcl)
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ ماكو شي مشتغل يحمار", show_alert=True)
...
async def cbmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.mute_stream(chat_id)
            await query.edit_message_text(
                "🔇تم كتم صوت حساب مساعد بنجاح", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
...
async def cbunmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.unmute_stream(chat_id)
            await query.edit_message_text(
                "🔊 تم الغاء كتم حساب المساعد", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
